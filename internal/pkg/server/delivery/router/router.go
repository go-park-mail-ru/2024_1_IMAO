...
func NewRouter(pool *pgxpool.Pool, logger *zap.SugaredLogger) *mux.Router {
	...
	router.HandleFunc("/api/adverts/{city:[a-zA-Z]+}/{category:[a-zA-Z_]+}", advertsHandler.GetAdsList)
	router.HandleFunc("/api/adverts/{city:[a-zA-Z]+}/{category:[a-zA-Z_]+}/{id:[0-9]+}", advertsHandler.GetAdvert)
