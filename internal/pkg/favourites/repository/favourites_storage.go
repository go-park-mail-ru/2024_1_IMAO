...
func (favouritesStorage *FavouritesStorage) getFavouritesByUserID(ctx context.Context, tx pgx.Tx,
	...
	if err != nil {
		logging.LogError(logger,
			fmt.Errorf("something went wrong while executing select adverts query, err=%w", err))
...
func (favouritesStorage *FavouritesStorage) deleteAdvByIDs(ctx context.Context, tx pgx.Tx, userID uint,
	...
	if err != nil {
		logging.LogError(logger,
			fmt.Errorf("something went wrong while executing advert delete from the favourite, err=%w", err))
		favouritesStorage.metrics.IncreaseErrors(funcName)
...
func (favouritesStorage *FavouritesStorage) DeleteAdvByIDs(ctx context.Context, userID uint, advertID uint) error {
	...
	if err != nil {
		logging.LogError(logger, fmt.Errorf("something went wrong while getting adverts list, most likely , err=%w", err))
...
func (favouritesStorage *FavouritesStorage) AppendAdvByIDs(ctx context.Context, userID uint, advertID uint) bool {
	...
	err = pgx.BeginFunc(ctx, favouritesStorage.pool, func(tx pgx.Tx) error {
		addedInner := favouritesStorage.appendAdvByIDs(ctx, tx, userID, advertID)
		added = addedInner
	...
	if err != nil {
		logging.LogError(logger, fmt.Errorf("error while executing addvert add to favourites, err=%w", err))
	}

	return added
}

func (favouritesStorage *FavouritesStorage) getSubscribedAdvertsByUserID(ctx context.Context, tx pgx.Tx,
	userID uint) ([]*models.ReturningAdInList, error) {
	funcName := logging.GetOnlyFunctionName()
	logger := logging.GetLoggerFromContext(ctx).With(zap.String("func", logging.GetFunctionName()))
	...
	INNER JOIN city c ON a.city_id = c.id
	INNER JOIN category ON a.category_id = category.id
	INNER JOIN subscription s ON a.user_id = s.user_id_merchant
	WHERE s.user_id_subscriber = $1 AND a.advert_status = 'Активно'
	ORDER BY a.id;
	`
...
func (favouritesStorage *FavouritesStorage) GetSubscribedAdvertsByUserID(ctx context.Context,
	userID uint) ([]*models.ReturningAdInList, error) {
	logger := logging.GetLoggerFromContext(ctx).With(zap.String("func", logging.GetFunctionName()))
	...
	err := pgx.BeginFunc(ctx, favouritesStorage.pool, func(tx pgx.Tx) error {
		favouritesInner, err := favouritesStorage.getSubscribedAdvertsByUserID(ctx, tx, userID)
		favourites = favouritesInner
